...
export default function SimulationStudio() {
  ...
  const chartOption = !fpp && run.traces?.length ? {
    ...
    xAxis: { type: 'category', data: run.convergence.labels, name: fpp ? text('迭代', 'Iteration') : text('代理模型', 'Surrogate'), nameLocation: 'middle', nameGap: 28 },
    yAxis: { type: 'value', name: text('误差 / 残差', 'Error / residual'), min: 0 },
    series: [{ name: text('报告值', 'Reported value'), type: fpp ? 'line' : 'bar', data: run.convergence.values, symbolSize: 9, barMaxWidth: 55, lineStyle: { width: 3 } }, ...(run.convergence.threshold === null ? [] : [{ name: text('配置阈值', 'Configured threshold'), type: 'line', data: run.convergence.labels.map(() => run.convergence.threshold), symbol: 'none', lineStyle: { type: 'dashed', width: 1.5 } }])],
  };
  ...
  return <main className="simStudio">
    <header className="simHeading"><div><p className="simEyebrow">FUSIONDIGITAL / SIMULATION STUDIO</p><h1>{text('仿真模拟', 'Simulations')}<span>FUSE</span></h1><p>{text('从计算工况，到可追溯的物理与工程结果。', 'From a defined scenario to traceable physics and engineering results.')}</p></div><div className="simWorkspaceActions"><span>{text('结果回放与研究准备', 'Result replay & study preparation')}</span><input ref={importRef} type="file" accept=".json,application/json" hidden onChange={e => void importResult(e.target.files?.[0])} /><button className="simButton" disabled={importing} onClick={() => importRef.current?.click()}>{importing ? text('校验中…', 'Validating…') : text('导入结果', 'Import result')}</button><button hidden={domain !== 'physics'} className="simButton" onClick={() => downloadJson(run, `${run.id}.json`)}>{text('导出结果', 'Export result')}</button><button hidden={domain !== 'physics'} className="simButton simButtonPrimary" onClick={() => { setTab('parameters'); setDomain('physics'); }}>{text('配置研究', 'Configure study')} ↗</button></div></header>
    ...
    <div className="simCaseSwitcher"><label>{text('计算工况', 'Run')}<select aria-label={text('选择计算结果', 'Select simulation result')} value={run.id} onChange={e=>{setSelectedId(e.target.value);setTab('overview');}}>{runs.map(item=><option value={item.id} key={item.id}>{item.caseId==='fpp-stationary'?'FPP':'DIII-D'} · {item.id} {importedIds.includes(item.id)?text('· 来源待核验','· unverified'):''}</option>)}</select></label><span>{run.engine.id} {run.engine.version} · {run.engine.runtime.name} {run.engine.runtime.version}</span><span>SIMULATED · {text('装置资格：未验证','Device: Not validated')}</span></div>
