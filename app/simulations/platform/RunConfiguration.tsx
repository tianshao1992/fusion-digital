...
import { parseTransportGeometry, type TransportGeometry } from './geometry';
import { download } from './display';

export default function RunConfiguration({ result, en, onCollected }: { result: TransportResult; en: boolean; onCollected: (result: TransportResult, geometry: TransportGeometry | null) => void }) {
  ...
  const [completed, setCompleted] = useState<{ result: TransportResult; geometry: TransportGeometry | null } | null>(null);
  ...
  async function refresh(id: string, signal?: AbortSignal) {
    ...
    if (next.state === 'succeeded' && next.processStopped) {
      ...
      let geometry: TransportGeometry | null = null;
      try {
        geometry = parseTransportGeometry(await request(`/v1/jobs/${id}/geometry`, { signal }));
        if (geometry.runId !== id || geometry.sourceNativeSha256 !== collected.provenance.nativeSha256) throw new Error('GEOMETRY_IDENTITY');
      } catch (error) {
      ...
      setCompleted({ result: collected, geometry });
    }
    setJob(next);
  }
  useEffect(() => {
  ...
  async function importFile(file: File | undefined, profileOnly = false) {
    ...
    setNotice(t('文件已校验，启动时上传至所选计算节点。', 'File validated; it will be uploaded when you start.'));
  }
  ...
  return <section className="transportPanel transportCompute"><h3>{t('配置与启动计算', 'Configure & start computation')}</h3><p>{t('参数和输入数据发送至计算节点；代码、环境和原始文件保留在节点上。', 'Parameters and input data go to the compute node; code, environment and native files stay there.')}</p>
    ...
    {result.recipe === 'fuse-profile-handoff' && <div className="transportInputSummary"><label className="transportUpload">{t('上传 Te / Ti / ne 剖面 JSON', 'Upload Te / Ti / ne profile JSON')}<input type="file" accept="application/json,.json" disabled={locked} onChange={e => { const file = e.target.files?.[0]; e.target.value = ''; void action(() => importFile(file, true)); }} /></label><p>{t('固定 DIII-D-like 圆截面接收模型；只传递模拟剖面，不传递磁平衡。温度 eV，密度 m⁻³，ρtor,norm 从 0 到 1。', 'Fixed DIII-D-like circular receiving model; simulated profiles only, without equilibrium. Temperature eV, density m⁻³, ρtor,norm from 0 to 1.')}</p>{snapshot && <p><code>{snapshot.source.runId}</code> · {snapshot.rho.length} {t('径向点', 'radial points')}</p>}<button className="transportButton" disabled={!connected || locked} onClick={() => action(async () => setSnapshot(parseProfileSnapshot(await request('/v1/inputs/fuse-profile'))))}>{t('读取节点 FUSE 剖面', 'Load node FUSE profiles')}</button></div>}
    ...
    {completed && <div className="transportControls"><button className="transportButton primary" onClick={() => onCollected(completed.result, completed.geometry)}>{t('查看新计算结果', 'Explore new results')}</button><button className="transportButton" onClick={() => download(`${completed.result.id}.json`, JSON.stringify(completed.result, null, 2))}>{t('下载完整结果', 'Download full result')}</button></div>}{notice && <p className="transportNotice" role="status">{notice}</p>}
  </section>;
}
