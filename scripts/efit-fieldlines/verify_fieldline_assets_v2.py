...
def verify(directory: Path, audit_path: Path):
    ...
    assert index["schemaVersion"] == "fusion.efit.fieldlines.v2"
    ...
    for shot in index["shots"]:
        ...
        for part in shot["chunks"]:
            ...
            assert len(compressed) < 8_000_000
            raw = gzip.decompress(compressed)
            assert len(raw) < 16_000_000 and not any(word in raw for word in forbidden)
        ...
        max_drift = 0
        max_q_relative = max_loop_closure = 0
        contour_counts = {}
        max_points_per_line = 0
        all_antenna_samples = 0
        for k, (frame, summary) in enumerate(zip(frames, shot["frames"])):
            ...
            assert frame["efitScalars"] == summary["efitScalars"]
            scalars = frame["efitScalars"]
            assert scalars is not None and all(math.isfinite(value) for value in scalars.values() if value is not None)
            assert set(scalars) == {"currentA", "rAxisM", "zAxisM", "bcentrT", "psiAxisWb", "psiBoundaryWb", "q95"}
            assert scalars["psiAxisWb"] != scalars["psiBoundaryWb"]
            contour_counts[len(frame["efitContours"])] = contour_counts.get(len(frame["efitContours"]), 0) + 1
            assert len(frame["efitContours"]) >= 1
            assert frame["efitContours"][-1]["psiN"] == 1
            for contour in frame["efitContours"]:
                assert contour["closed"] and contour["psiN"] in [.25, .5, .75, .9, .97, 1]
                points = contour["pointsRzM"]
                assert 8 <= len(points) <= 512 and len(points) % 2 == 0
                assert points[:2] == points[-2:] and all(math.isfinite(value) for value in points)
            profile = frame["antennaMidplaneFlux"]
            assert profile["state"] == "valid" and profile["rStartM"] == 1.1 and profile["rStepM"] == .001
            assert profile["zM"] == 0 and profile["phiDegrees"] == 300
            assert len(profile["psiWb"]) == 501 and all(value is None or math.isfinite(value) for value in profile["psiWb"])
            assert abs(profile["psiAxisWb"] - scalars["psiAxisWb"]) < 1e-12
            assert abs(profile["psiBoundaryWb"] - scalars["psiBoundaryWb"]) < 1e-12
            for start, end in profile["lcfsIntervalsRM"]:
                assert 0 < start < end < 3
            all_antenna_samples += len(profile["psiWb"])
            ...
            for line, pn in zip(frame["lines"], index["seedPsiN"]):
                ...
                points = line["points"]
                assert 511 * 3 <= len(points) <= 4095 * 3 and len(points) % 3 == 0
                max_points_per_line = max(max_points_per_line, len(points) // 3)
                ...
                assert all(abs(phi) <= 210 for phi in points[1::3])
                assert all(abs(z) <= 1.901 for z in points[2::3])
                assert 255 <= line["seedIndex"] <= 2047
                assert abs(points[line["seedIndex"] * 3 + 1] - 5 * math.pi / 3) < 1e-6
                assert len(line["termination"]) == len(line["arcLengthM"]) == 2
                assert all(reason == "poloidal-coverage-complete" for reason in line["termination"])
                assert all(0 < length <= 300.001 for length in line["arcLengthM"])
                assert all(abs(angle - math.pi) < 1e-6 for angle in line["poloidalSpanRad"])
                assert all(0 < angle <= 32 * 2 * math.pi for angle in line["toroidalSpanRad"])
                assert abs(line["qTrace"] - sum(line["toroidalSpanRad"]) / (2 * math.pi)) < 1e-8
                assert abs(line["qTrace"] - abs(line["sourceQ"])) <= max(.05, .05 * abs(line["sourceQ"])) + 1e-8
                assert line["poloidalClosureErrorM"] <= 1e-4
                max_loop_closure = max(max_loop_closure, line["poloidalClosureErrorM"])
                max_q_relative = max(max_q_relative, line["qRelativeDifference"])
        ...
        metrics.append({"shot": shot["shot"], "validFrames": valid, "unavailableFrames": unavailable,
                        ...
                        "maxPsiNDrift": max_drift, "maxQRelativeDifference": max_q_relative,
                        "maxPoloidalClosureErrorM": max_loop_closure, "maxPointsPerLine": max_points_per_line,
                        "contourCountFrames": contour_counts, "antennaSamples": all_antenna_samples})
