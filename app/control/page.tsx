  ...
  return <main className="controlPage">
    ...
    <header className="controlHero">
      <div className="controlHeroCopy">
        ...
        <h1>集成控制</h1>
        <p>查找控制任务、装置 PCS、闭环实验，以及论文和代码。</p>
        <div className="controlActions">
          <a href="#research">检索关键工作</a>
          <a href="/fusion-integrated-control-research-report.docx" download>下载完整报告</a>
        </div>
    ...
    <section className="controlThesis">
      ...
      <h2>集成控制协调不同时间尺度的<span>状态、目标、约束、执行器、事件与责任。</span></h2>
      <div><p>控制任务调节装置状态，PCS 按时限、配置与权限执行；孪生负责状态同步、适用域、预测—实测残差、版本和验证证据。</p><p>按控制对象与证据检索，再查装置上的实时、影子、闭环或常规运行记录。</p></div>
    </section>

    <section className="controlArchitecture" id="architecture">
      <div className="controlSectionHead"><p className="controlIndex">01 / CLOSED-LOOP ARCHITECTURE</p><h2>六类信息 · 两条权威链</h2><p>诊断与设备数据形成状态，任务控制提出请求，监督/分配层协调执行器与约束，PCS 确定性执行。孪生旁路预测、回放和更新证据，不替代可共享状态的独立机器保护/安全链。</p></div>
      <figure><img src="/figures/control-pcs-layers-nature.png" alt="托卡马克PCS分层、数字孪生和独立保护的关系" loading="lazy" decoding="async"/><figcaption>快内环、任务控制、慢监督与脉冲编排各有时间预算和回退策略。</figcaption></figure>
    ...
    <section className="controlTasks" id="tasks">
      <div className="controlSectionHead"><p className="controlIndex">02 / T0–T9 TASK TAXONOMY</p><h2>十类控制任务</h2><p>T0 状态估计与 T9 PCS/V&amp;V 支撑全部任务；T1–T8 覆盖启动/磁通、位形、剖面、不稳定性、排热、功率、异常终止与多执行器集成。</p></div>
    ...
    <section className="controlResearch" id="research">
      <div className="controlSectionHead controlResearchHead"><div><p className="controlIndex">03 / SEARCHABLE RESEARCH LANDSCAPE</p><h2>控制研究目录</h2><p>聚合问题、架构、传感器、执行器、装置、验证、结果、局限、论文与代码；装置数据、实时运行和真机闭环分别标注。</p></div><div className="controlDownloads"><a href="/fusion-integrated-control-research-report.docx" download><b>WORD</b><span>完整技术报告</span></a><a href="/data/fusion-control-landscape.json" download><b>JSON</b><span>控制工作数据</span></a><a href="/fusion-control-paper-code-index.csv" download><b>CSV</b><span>论文代码索引</span></a><a href="/fusion-control-references.bib" download><b>BIB</b><span>引用元数据</span></a></div></div>
      <ControlResearchCatalog />
    </section>

    <section className="controlEvidence" id="evidence">
      <div className="controlSectionHead"><p className="controlIndex">04 / EVIDENCE &amp; VERIFICATION</p><h2>科学证据 E · 工程部署 D</h2><p>升级同时验证模型、数据、软件运行时、硬件接口和故障回退；单次最优曲线不代表实时或安全资格。</p></div>
    ...
    <section className="controlGaps">
      <div className="controlSectionHead"><p className="controlIndex">05 / FROM INTEGRATED CONTROL TO DIGITAL TWIN</p><h2>八项运行能力</h2><p>持续运行需明确装置状态、模型可信度、动作审批、结果检验与版本发布责任。</p></div>
    ...
    <section className="controlRoadmap" id="roadmap">
      <div className="controlSectionHead"><p className="controlIndex">06 / FUSIONDIGITAL ROADMAP</p><h2>从 DINA / MEQ 到电厂控制</h2><p>影子模式先于闭环，多任务协调先验证可解释与可回退，再接入燃烧、工程限值、RAMI、氚、热循环和电网目标。</p></div>
    ...
    <section className="controlClosing"><div><p className="controlIndex">07 / NEXT</p><h2>平台实施</h2></div><div><Link href="/platform#contracts">查看统一数据契约与技术路线 →</Link></div></section>
    <SiteFooter />
  </main>;
}
