...
网页另提供 [DINA/FGE 历史示例回放](../../docs/CONTROL_EXAMPLES.md)，采用独立的
`control-example.v1` 合同，不作为本轮云端运行或 TORAX 输入资格证据。
...
两套引擎的新运行默认 **200 ms（0.2 s）**，即 200 个 1 ms STEP；加上 RESET，
完整结果必须包含 201 个采样点（相对时间 0–200 ms）。这不是修改初态时间：
FGE 仍从 400 ms 初态启动，DINA 仍从 800 ms 初态启动。
网页与导出运行配置共用同一默认值；实际时长由提交的 runspec 固定。
默认值变更不代表求解器已完成该时长：公开历史回放仍分别为 DINA 16 ms、FGE 100 ms，
必须在计算节点重新运行并审核完整结果后才能发布 200 ms 示例。不得拉伸时间轴、补帧，
或把长运行的截取窗口标为独立成功运行；未完成的请求保留失败、取消或超时状态与实际步数。
