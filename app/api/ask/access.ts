...
export async function authorizeAsk(input: { requestedTokens: number; provider: string; model: string; questionLength: number; contextEntries: number; historyTurns?: number; conversationId?: string; principal?: Principal | null }): Promise<AskAccess> {
  ...
  try {
    ...
    await safeAudit({ actorUserId: principal.user.id, requestId, outcome: "success", action: "knowledge.ask.reserve", provider: input.provider, model: input.model });
    ...
    if (reason instanceof Error && (reason as Error & { code?: string }).code === "ACCOUNT_INACTIVE") throw reason;
...
export async function settleAsk(access: AskAccess, input: { status: "succeeded" | "failed" | "cancelled"; inputTokens?: number; outputTokens?: number; provider: string; model: string }) {
  ...
  try {
    ...
    await safeAudit({ actorUserId: access.userId, requestId: access.requestId, outcome: input.status === "succeeded" && settled ? "success" : "failure", action: "knowledge.ask.settle", provider: input.provider, model: input.model });
  } catch (reason) {
...
async function safeAudit(input: { actorUserId: string; requestId: string; outcome: "success" | "failure"; action: string; provider: string; model: string }) {
  try {
    const { appendAuditEvent } = await import("@/db/audit");
    await appendAuditEvent({ actorUserId: input.actorUserId, requestId: input.requestId, action: input.action, resourceType: "knowledge_answer", resourceId: `${input.provider}:${input.model}`, outcome: input.outcome });
  } catch (reason) {
