...
两引擎使用一致的三维结果卡片、色板、视角与剖开角度、R–Z 云图剖切、磁面层级选择和 q 约束轨迹控件；场变量在各自的二维与三维视图间联动。色板一致仅是显示约定，不表示两引擎的数值、装置或物理坐标相同。

- FUSE 磁面来自该运行导出的闭合 ψN 轮廓；归档平衡网格或有界剖面映射决定二维 R–Z 与三维剖切的数值。三维表面是轴对称旋转显示，不是新的三维平衡求解。
- TORAX 的原生输出是时间—径向剖面；三维表面来自同运行、固定参数化 R–Z 重建轮廓，径向坐标为 ρtor,N 而非 ψN。R–Z 云图和三维剖切使用同批映射单元；切到“时间—半径”二维模式时使用的是另一批原生时空样本，仅与三维共用场变量、单位和色标。当前时刻色标包含可选三维磁面的原生剖面值，避免边界磁面读数落在色标之外。
- 重建轮廓须同运行、同源摘要且全部闭合、嵌套；相邻层连接的剖切单元不得折叠或自交。资格不满足时不绘制派生 R–Z / 三维云图，原生时间—径向结果仍可查看。STEP 输入 ψ 网格可显示二维 R–Z 场，但没有已审定的闭合三维重建磁面，不能自动旋转碎片等值线补足。
- TORAX 时间回放改变剖面数值和 q 轨迹，固定几何不随时间改变；Three.js 场景复用画布并增量更新场、剖切和几何。缺失数据保持不可用，不用零值或其他引擎的结果代替。
