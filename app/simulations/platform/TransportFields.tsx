...
import { download, label, number } from './display';
import { selectableTransportRings, transportColorScale } from './surface-display';
import TransportFieldLines from './TransportFieldLines';

export default function TransportFields({ result, timeIndex, en, onTime, liveGeometry, liveGeometrySha256 }: { result: TransportResult; timeIndex: number; en: boolean; onTime: (index: number) => void; liveGeometry?: TransportGeometry | null; liveGeometrySha256?: string | null }) {
  ...
  const geometrySha256 = entry?.artifact.rawSha256 ?? (liveGeometry?.runId === result.id && liveGeometry.sourceNativeSha256 === result.provenance.nativeSha256 ? liveGeometrySha256 ?? null : null);
  useEffect(() => {
    if (!entry) return;
  ...
  const shapeQualified = useMemo(() => geometry?.kind !== 'shape-reconstruction' || selectableTransportRings(geometry, result).length > 0, [geometry, result]);
  const cells = useMemo(() => mode === 'time' ? spaceTimeCells(result, selected) : geometry && shapeQualified ? crossSectionCells(geometry, result, selected, timeIndex) : [], [mode, result, selected, geometry, shapeQualified, timeIndex]);
  const colorScale = useMemo(() => transportColorScale(result, selected, cells, fixedScale,
    mode === 'section' && geometry?.kind === 'shape-reconstruction' && shapeQualified ? timeIndex : undefined),
    [result, selected, cells, fixedScale, mode, geometry, shapeQualified, timeIndex]);
  const { scale, unit, minimum, maximum } = colorScale;
  ...
  const sectionAvailable = !!geometry && !error && shapeQualified;
  ...
  return <><TransportFieldLines result={result} geometry={sectionAvailable ? geometry : null} geometrySha256={geometrySha256} timeIndex={timeIndex} field={selected} onFieldChange={value => { setField(value); setSample(0); }} colorScale={colorScale} scaleAcrossTime={fixedScale || mode === 'time'} twoDMode={mode} en={en} /><section className="transportPanel transportFieldPanel"><div className="transportFieldHeading"><div><p className="transportEyebrow">2D FIELD VIEW</p><h3>{t('二维云图', 'Two-dimensional fields')}</h3></div><span className="transportTag">{description}</span></div>
