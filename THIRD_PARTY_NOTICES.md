...
The project operator subsequently and explicitly authorized public web delivery of separate simplified browser geometry derivatives under `public/models/exl50u-interactive/`. This authorization applies only to the declared standard and higher-detail browser LODs. The original STEP/PPTX, B-Rep topology, dimensional annotations, full 1,518-definition/16,748-occurrence assembly tree, engineering materials and manufacturing tolerances remain private. Both LODs merge the source assembly into the same twelve selectable system-level meshes. The higher-detail LOD uses finer absolute tessellation, feature-aware normals and Meshopt transport compression to improve browser silhouettes and surface shading; it does not restore engineering surface definitions or dimensional authority. The manifest and [`public/licenses/EXL50U-PUBLIC-DERIVATIVE.txt`](public/licenses/EXL50U-PUBLIC-DERIVATIVE.txt) record the public-display boundary, hashes, byte sizes, coordinate conversion and prohibited engineering uses.
