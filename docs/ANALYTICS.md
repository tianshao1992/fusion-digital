...
初始站点所有者的首次授权由所有者明确批准的 custom D1 migration 完成：它只匹配
批准时读取到的精确 Sites 内部 `user_id`，仅在该账户仍为 `active` 时生效，并写入
`actor_user_id = NULL` 的带外授权审计事件。该迁移不是通用 bootstrap 或角色管理接口；
后续授权、撤销或更换管理员仍须在受控 D1 管理面执行，不得复制迁移、改用邮箱匹配或
增加运行时自动提权逻辑。
