...
export default function FuseWorkspace() {
  ...
  return <main className="simStudio">
    ...
    <div className="simCaseSwitcher"><label>{text('计算工况', 'Run')}<select aria-label={text('选择计算结果', 'Select simulation result')} value={run.id} onChange={e=>{setSelectedId(e.target.value);setTab('overview');}}>{runs.map(item=><option value={item.id} key={item.id}>{item.caseId==='fpp-stationary'?'FPP':item.caseId==='diiid-stationary'?'DIII-D · coupled':`DIII-D · ${item.convergence.labels.join('/')}`} · {item.id} {sessionIds.includes(item.id)?text('· 会话计算','· session run'):importedIds.includes(item.id)?text('· 来源待核验','· unverified'):''}</option>)}</select></label><span>{run.engine.id} {run.engine.version} · {run.engine.runtime.name} {run.engine.runtime.version}</span><span>SIMULATED · {text('装置资格：未验证','Device: Not validated')}</span></div>
