...
- OpenAI Sites 仅承载显式创建的平台预览，以及 SIWC / D1 协作能力
...
Codeup `master` 是人和 Codex 的唯一协作事实源，GitHub `main` 是同一提交的公开代码镜像。正式发布把同一个完整提交 SHA 部署到阿里云香港；失败时只回滚香港不可变 release。Sites 仅在用户明确要求预览时使用，其短期源仓库凭证不得长期保存或共享。
...
- Sites 预览不属于生产发布门禁，也不得触发生产 DNS 修改
- 公开文件不含敏感参数或凭证
- 报告下载与结构化数据均可访问
