...
const englishContent: Record<string, string> = {
  ...
  '能力足够的桌面客户端可自动加载高精度模型': 'High detail auto-loads on capable desktop clients',
  '20 个匿名高精度运输分片（270,978,652 B / 271.0 MB）': '20 anonymous high-detail transport shards (270,978,652 B / 271.0 MB)',
  '仅保留高精度 · Meshopt + GPU instancing': 'High detail only · Meshopt + GPU instancing',
  'EXL‑50U 装置集成总装的公开匿名高精度数字样机，用于整机外观、旋转缩放、透明度与剖切浏览。': 'Public anonymous high-detail digital prototype of the EXL‑50U integrated general assembly for whole-device appearance, orbit and zoom, opacity and clipping.',
  '20 个匿名高精度运输分片 · 270,978,652 B（271.0 MB）· 摘要锁定串行加载 · 无标准预览或回退': '20 anonymous high-detail transport shards · 270,978,652 B (271.0 MB) · digest-locked serial loading · no standard preview or fallback',
  '加载经授权、脱敏、简化和逐文件复核的公开匿名总装高精度可视化派生。能力足够的桌面客户端可以自动串行加载 20 个高精度运输分片；窄屏、省流量或低内存客户端必须由用户显式启动。当前发布不提供标准预览或降级回退，任一分片下载、完整性或解码失败均关闭本次模型加载。公开根和运输分片不表达 BOM、工程系统、源装配树、材料、PMI、尺寸标注、权威尺寸表或配置权威；可视几何保留近似米制尺度，仅用于外观展示，不能作为测量或工程尺寸依据；原始 STEP 与工程 CAD 不会由网站下发。': 'Loads an authorized, sanitized, simplified and per-file-reviewed public anonymous high-detail visualization derivative of the general assembly. Capable desktop clients can automatically load the 20 high-detail transport shards serially; narrow-screen, data-saving or low-memory clients require an explicit launch. This release provides no standard preview or downgrade fallback, and any shard download, integrity or decode failure closes the model load. The public root and transport shards encode no BOM, engineering systems, source assembly tree, materials, PMI, dimension annotations, authoritative dimension tables or configuration authority. The visualization retains an approximate metre-scale envelope for appearance only and must not be used for measurement or engineering dimensions; source STEP and engineering CAD are never delivered by the site.',
