...
import { defaultRunSpec } from './run-spec';
import { useFuseRemoteExecution } from './FuseRemoteExecution';
import type { FuseCollectedResult } from './fuse-remote';
import type { FluxCoordinateMap } from './flux-coordinate-map';
import type { PhysicsData } from './physics';

export default function FuseWorkspace() {
  ...
  const [runSpec, setRunSpec] = useState(defaultRunSpec);
  ...
  const [sessionScience, setSessionScience] = useState<Record<string, { physics: PhysicsData; coordinateMap?: FluxCoordinateMap }>>({});
  ...
  const acceptRemoteResult = useCallback((result: FuseCollectedResult, activate: boolean) => {
    const existing = runs.find(item => item.id === result.run.id);
    if (existing && existing.source.recordSha256 !== result.run.source.recordSha256) {
      setNotice(en ? 'Run ID collision: the gateway result was not added to this session.' : '运行 ID 冲突：网关结果未加入当前会话。');
      return;
    }
    if (!existing && runs.length >= 32) {
      setNotice(en ? 'This session already holds 32 results. Download the gateway result before refreshing.' : '当前会话已保留 32 份结果，请先下载网关结果再刷新页面。');
      return;
    }
    if (!existing) setRuns(current => [result.run, ...current]);
    setSessionIds(current => current.includes(result.run.id) ? current : [...current, result.run.id]);
    setSessionScience(current => ({ ...current, [result.run.id]: { physics: result.physics, ...(result.coordinateMap ? { coordinateMap: result.coordinateMap } : {}) } }));
    if (activate) { setSelectedId(result.run.id); setDomain('physics'); setTab('overview'); }
    setNotice(en ? 'Gateway-verified safe projections are available only in this browser session; refresh will discard them.' : '网关已核验的安全投影仅载入当前浏览器会话；刷新页面后即丢失。');
  }, [en, runs, setNotice]);
  const remote = useFuseRemoteExecution(en, acceptRemoteResult);
  const run = runs.find(r => r.id === selectedId) ?? runs[0];
  const sessionResult = sessionScience[run.id];
  const imported = importedIds.includes(run.id);
  const transient = sessionIds.includes(run.id);
  const trusted = !imported && !transient;
  ...
  return <main className="simStudio">
    <header className="simHeading"><div><p className="simEyebrow">FUSIONDIGITAL / SIMULATION ENGINES</p><h1>{text('仿真引擎', 'Simulation Engines')}<span>FUSE</span></h1><p>{text('查看可追溯的物理与工程结果。', 'View traceable physics and engineering results.')}</p></div><div className="simWorkspaceActions"><span>{remote.active ? text(`FUSE 任务 ${remote.job?.state} · 切换视图不影响轮询`, `FUSE job ${remote.job?.state} · polling continues across views`) : text('结果回放与鉴权计算', 'Result replay & authenticated compute')}</span><input ref={importRef} type="file" accept=".json,application/json" hidden onChange={e => void importResult(e.target.files?.[0])} /><button className="simButton" disabled={importing} onClick={() => importRef.current?.click()}>{importing ? text('校验中…', 'Validating…') : text('导入结果', 'Import result')}</button><button hidden={domain !== 'physics'} className="simButton" onClick={() => downloadJson(run, `${run.id}.json`)}>{text('导出结果', 'Export result')}</button><button hidden={domain !== 'physics'} className="simButton simButtonPrimary" onClick={() => { setTab('engine'); setDomain('physics'); }}>{remote.active ? text('查看计算任务', 'Open compute job') : text('配置与启动计算', 'Configure & run')} ↗</button></div></header>
    <div className="simDomainBar" role="group" aria-label={text('仿真领域', 'Simulation domain')}><button data-agent-safe="click" aria-pressed={domain === 'physics'} onClick={() => setDomain('physics')}><span>01</span>{text('物理模拟', 'Physics')}</button><button data-agent-safe="click" aria-pressed={domain === 'engineering'} onClick={() => setDomain('engineering')}><span>02</span>{text('工程仿真', 'Engineering')}</button><p>{text('集成设计工作台', 'Integrated design workspace')}</p><Link href={domain === 'physics' ? '/physics' : '/engineering'}>{text('领域知识', 'Domain knowledge')} ↗</Link></div>
    <p className="simFeedback" role="status" aria-live="polite">{notice}</p>
    {remote.job && <div className="simActiveJobBar"><span className={remote.job.state==='succeeded'?'simGood':remote.active?'':'simWarning'}>FUSE · {remote.job.state}</span><code>{remote.job.id}</code><button className="simTextButton" onClick={()=>{setDomain('physics');setTab('engine');}}>{text('任务控制', 'Job controls')} ↗</button></div>}
    <div className="simCaseSwitcher"><label>{text('计算工况', 'Run')}<select data-agent-safe="select" aria-label={text('选择计算结果', 'Select simulation result')} value={run.id} onChange={e=>{setSelectedId(e.target.value);setTab('overview');}}>{runs.map(item=><option value={item.id} key={item.id}>{item.caseId==='fpp-stationary'?'FPP':item.caseId==='diiid-stationary'?'DIII-D · coupled':`DIII-D · ${item.convergence.labels.join('/')}`} · {item.id} {sessionIds.includes(item.id)?text('· 会话计算','· session run'):importedIds.includes(item.id)?text('· 来源待核验','· unverified'):''}</option>)}</select></label><span>{run.engine.id} {run.engine.version} · {run.engine.runtime.name} {run.engine.runtime.version}</span><span>SIMULATED · {text('装置资格：未验证','Device: Not validated')}</span></div>
    <div className="simGrid">
      <section className="simCanvas" aria-label={text('计算结果', 'Simulation results')}>
        {domain === 'engineering' ? <PhysicsWorkbench run={run} en={en} engineering trusted={trusted} directData={sessionResult?.physics} directCoordinateMap={sessionResult?.coordinateMap} /> : <>
        <div className="simTabs" role="group" aria-label={text('工作台视图', 'Workspace view')}>{([['overview', '结果概览', 'Overview'], ['engine', '计算引擎', 'Engine'], ['parameters', '设计草稿', 'Design draft'], ['data', '数据与输出', 'Outputs'], ['comparison', '结果对照', 'Compare'], ['provenance', '版本与证据', 'Evidence'], ['guide', '运行指引', 'Run guide']] as const).map(([key, zh, english]) => <button data-agent-safe="click" key={key} aria-pressed={tab === key} onClick={() => setTab(key)}>{text(zh, english)}</button>)}</div>
        {imported && <p className="simNotice">{text('导入结果的来源待核验；下方展示该文件声明的执行与评估信息。', 'Imported source not verified; execution and assessment below are claims from the supplied record.')}</p>}
        {transient && <p className="simNotice">{text('此结果由鉴权网关收集并在本次浏览器会话中通过数据契约与身份字段校验；尚未纳入版本化目录，刷新即丢失。原生 HDF5、日志和模型仍在计算节点。', 'This result was collected by the authenticated gateway and checked for contracts and identity fields in this browser session. It is not in the versioned catalog and disappears on refresh. Native HDF5, logs and models remain on the compute node.')}</p>}
        {tab === 'engine' ? <EngineWorkspace en={en} spec={runSpec} setSpec={setRunSpec} remote={remote}/> : tab !== 'overview' ? <SimulationPanels tab={tab} run={run} runs={runs} en={en} draft={draft} setDraft={setDraft} /> : <>
