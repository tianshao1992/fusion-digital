...
export default function FluxSurfaceDemo({ data, coordinateMap, field, onFieldChange, en }: Props) {
  ...
  const [fieldLineCopies, setFieldLineCopies] = useState<(typeof FIELDLINE_COPIES)[number]>(2);
  const [fieldLineXray, setFieldLineXray] = useState(true);
  const [attempt, setAttempt] = useState(0);
  const [status, setStatus] = useState<'loading' | 'ready' | 'failed'>('loading');
  const host = useRef<HTMLDivElement>(null);
  const reset = useRef<(() => void) | null>(null);
  const pose = useRef<CameraPose | null>(null);
  const fieldLineOverlay = useRef<SimulationFieldLineOverlay | null>(null);
  const renderScene = useRef<(() => void) | null>(null);
  const fieldLines = useMemo(() => buildFuseQFieldLines(data), [data]);
  const fieldLineView = useMemo(() => ({ identity: `${data.runId}:${data.timeSeconds}`, lines: showFieldLines ? fieldLines?.lines ?? [] : [], copies: fieldLineCopies, xray: fieldLineXray }), [data.runId, data.timeSeconds, showFieldLines, fieldLines, fieldLineCopies, fieldLineXray]);
  const currentFieldLineView = useRef(fieldLineView);
  const fieldLineOccluder = useRef<THREE.Mesh | null>(null);
  useEffect(() => {
    currentFieldLineView.current = fieldLineView;
    fieldLineOverlay.current?.setView(fieldLineView);
    if (fieldLineOccluder.current) fieldLineOccluder.current.visible = !fieldLineView.xray && fieldLineView.lines.length > 0;
    renderScene.current?.();
  }, [fieldLineView]);
  ...
  useEffect(() => {
    ...
    void Promise.all([import('three'), import('three/addons/controls/OrbitControls.js'), import('./SimulationFieldLineOverlay')]).then(([T, { OrbitControls }, { createSimulationFieldLineOverlay }]) => {
      ...
      cleanup = () => {
        ...
        fieldLineOverlay.current?.dispose();
        fieldLineOverlay.current = null;
      ...
      const occluder = new T.Mesh(geometry, depthOnly);
      occluder.renderOrder = 20;
      ...
      fieldLineOverlay.current = createSimulationFieldLineOverlay(scene, renderer);
      fieldLineOverlay.current.setView(currentFieldLineView.current);
      const resize = () => {
        const { width, height } = element.getBoundingClientRect();
        if (width < 1 || height < 1) return;
        renderer.setSize(width, height); camera.aspect = width / height; camera.updateProjectionMatrix(); fieldLineOverlay.current?.resize(width, height); render();
      };
  ...
  return <section className="fluxDemo" style={style} aria-label={en ? 'FUSE axisymmetric 3D field result' : 'FUSE 轴对称三维场结果'}>
    ...
    <div className="fluxLayout">
      <div className="fluxStage">
        ...
        <div className="fluxCorner"><span>{en ? fieldSpec.labelEn.toUpperCase() : fieldSpec.labelZh}</span><strong>{selectedField ? `${selectedField.value.toPrecision(4)} ${selectedField.unit}` : '—'}</strong><small>ψ<sub>N</sub> = {selection.value.toFixed(2)} · {en ? 'selected surface' : '当前磁通面'}</small>{showFieldLines && fieldLines && <small className="fluxLineQualifier">{en ? 'q-constrained display · not B-integrated' : 'q 约束示意 · 非 B 场积分'}</small>}</div>
      ...
      <aside className="fluxInspector">
        ...
        <div className="fluxFieldLineControls"><span className="fluxEyebrow">{en ? 'FIELD-LINE DIRECTION / q-CONSTRAINED' : '磁力线走向 / q 约束示意'}</span>
          <label className="fluxCheck"><input type="checkbox" checked={showFieldLines} disabled={!fieldLines} onChange={e => setShowFieldLines(e.target.checked)} />{en ? 'Show five-color trajectories' : '显示五色轨迹'}</label>
          <label className="fluxControl">{en ? 'Toroidal seed copies' : '环向播种副本'}<select value={fieldLineCopies} disabled={!fieldLines || !showFieldLines} onChange={e => setFieldLineCopies(Number(e.target.value) as (typeof FIELDLINE_COPIES)[number])}>{FIELDLINE_COPIES.map(count => <option key={count} value={count}>{count}×</option>)}</select></label>
          <label className="fluxCheck"><input type="checkbox" checked={fieldLineXray} disabled={!fieldLines || !showFieldLines} onChange={e => setFieldLineXray(e.target.checked)} />{en ? 'X-ray through surface' : '透视磁面'}</label>
          {fieldLines && <div className="fluxFieldLineLegend" aria-label={en ? 'q-constrained trajectory levels' : 'q 约束轨迹层级'}>{fieldLines.lines.map((line, index) => <span key={line.coordinateValue}><i style={{ backgroundColor: FIELDLINE_COLORS[index] }} />ψN {line.coordinateValue.toFixed(1)} · q {line.q.toFixed(2)}</span>)}</div>}
          {!fieldLines && <p className="fluxUnavailable">{en ? 'No complete, same-equilibrium closed contours and q profile; trajectory display is unavailable.' : '缺少同一平衡的完整闭合轮廓和 q 剖面，轨迹显示不可用。'}</p>}
        </div>
