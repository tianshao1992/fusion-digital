...
export default async function AccountPage() {
  if (isPublicAnonymousMode()) {
    const zh=<main className="accountPage">
      ...
      <header className="accountHero">
        <div className="accountHeroCopy">
          ...
          <h1>公开匿名访问。<br /><em>账户与密钥管理未启用。</em></h1>
          <div className="accountSignals" aria-label="公开匿名版边界"><span>无需登录</span><span>不识别用户身份</span><span>不保存个人密钥</span><span>仅开放公开内容</span></div>
        </div>
        <aside className="accountIdentityCard"><span>IDENTITY STATUS</span><strong>匿名访问</strong><p>账户功能未启用</p><small>继续浏览公开知识与数字样机</small></aside>
      </header>
      <section className="accountRegistration">
        <div><p>01 / PUBLIC ACCESS BOUNDARY</p><h2>此部署不提供登录、角色、配额与个人模型连接。</h2><p>本站不信任身份请求头，不提供登录或密钥入口。问答仅使用站内可核验资料。</p><Link className="accountPrimaryAction" href="/search">进入公开知识检索 <b>↗</b></Link></div>
    ...
    const en=<main className="accountPage">
      <SiteNav active="account" />
      <header className="accountHero"><div className="accountHeroCopy"><p>FUSIONDIGITAL / PUBLIC ANONYMOUS EDITION</p><h1>Public anonymous access.<br/><em>Accounts and credentials are disabled.</em></h1><div className="accountSignals" aria-label="Public anonymous deployment boundaries"><span>No sign-in required</span><span>No user identity processed</span><span>No personal credentials stored</span><span>Public content only</span></div></div><aside className="accountIdentityCard"><span>IDENTITY STATUS</span><strong>Anonymous access</strong><p>Account functions are disabled</p><small>Browse public knowledge and digital prototypes</small></aside></header>
      <section className="accountRegistration"><div><p>01 / PUBLIC ACCESS BOUNDARY</p><h2>Sign-in, roles, quotas and personal model connections are disabled.</h2><p>This site ignores identity headers and provides no sign-in or credential controls. Q&amp;A uses verifiable on-site material only.</p><Link className="accountPrimaryAction" href="/search">Open public knowledge search <b>↗</b></Link></div><ol><li><b>01</b><span><strong>Public browsing</strong> Access knowledge domains, facilities and digital prototypes.</span></li><li><b>02</b><span><strong>Anonymous retrieval</strong> Q&amp;A uses deterministic on-site retrieval only.</span></li><li><b>03</b><span><strong>Writes disabled</strong> Accounts, credentials and review operations are not exposed publicly.</span></li></ol></section>
      <SiteFooter />
    </main>;
  ...
  const zh=<main className="accountPage">
    ...
    {user ? <section className="accountWorkspace">
      <div className="accountWorkspaceIntro"><p>01 / ACCOUNT CONSOLE</p><div><h2>账户已建立。</h2><p>仅使用 ChatGPT 登录，不另存密码。数据库保存角色、配额、用量、审计及按用户隔离的加密模型连接。</p></div></div>
      ...
      <div><p>01 / REGISTER & SIGN IN</p><h2>使用 ChatGPT 注册与登录，无需新密码。</h2><p>首次登录自动建立账户，分配基础角色和每日额度。模型密钥仅存服务端，不返回浏览器。</p><a className="accountPrimaryAction" href={chatGPTSignInPath("/account")}>使用 ChatGPT 注册 / 登录 <b>↗</b></a></div>
    ...
    <section className="platformInlineLink"><span>角色、额度与写操作由服务端校验。</span><Link href="/platform#architecture">平台边界与路线 →</Link></section>
    <SiteFooter />
  </main>;
  const en=<main className="accountPage">
    <SiteNav active="account" />
    <header className="accountHero">
      <div className="accountHeroCopy"><p>FUSIONDIGITAL / ACCOUNT &amp; ACCESS</p><h1>Account and usage.<br/><em>Identity, access and audit.</em></h1><div className="accountSignals" aria-label="Account-system principles"><span>ChatGPT identity</span><span>Server-side role authorization</span><span>Daily usage controls</span><span>Complete audit trail</span></div></div>
