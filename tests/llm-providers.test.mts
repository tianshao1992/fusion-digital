...
import { requestProviderAnswer } from '../app/api/ask/provider-adapters.ts';
...
test('all provider adapters use fixed HTTPS endpoints, private auth headers and normalized output', async () => {
  const cases: Array<{
  ...
  }> = [
    {
      ...
      response: { output_text: groundedJson, usage: { input_tokens: 11, output_tokens: 7 } },
    ...
    },
    {
      ...
      response: { content: [{ type: 'text', text: groundedJson }], usage: { input_tokens: 12, output_tokens: 8 } },
      assertRequest(headers, body) {
    ...
    },
    {
      ...
      response: { choices: [{ message: { content: groundedJson } }], usage: { prompt_tokens: 13, completion_tokens: 9 } },
      assertRequest(headers, body) {
    ...
    },
    {
      ...
      response: { choices: [{ message: { content: groundedJson } }], usage: { prompt_tokens: 14, completion_tokens: 10 } },
      assertRequest(headers, body) {
  ...
  try {
    for (const entry of cases) {
      ...
      globalThis.fetch = async (url, init) => {
        assert.equal(String(url), entry.endpoint);
        assert.equal(init?.redirect, 'error');
        assert.equal(init?.cache, 'no-store');
        const headers = new Headers(init?.headers);
        const body = JSON.parse(String(init?.body)) as Record<string, unknown>;
        entry.assertRequest(headers, body);
        assert.doesNotMatch(String(init?.body), /secret-openai|secret-anthropic|secret-deepseek|secret-kimi/);
...
test('provider adapter rejects non-JSON and oversized upstream responses without exposing bodies', async () => {
  const resolution = resolveProvider('openai', { OPENAI_API_KEY: 'secret' });
  assert.equal(resolution.status, 'selected');
  if (resolution.status !== 'selected') throw new Error('provider was not selected');
  const originalFetch = globalThis.fetch;
  try {
