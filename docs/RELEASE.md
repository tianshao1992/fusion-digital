...
4. **隔离构建**：从该 SHA 建立 detached worktree，补齐并校验 ITER 运行时资产，
   ...
   `FUSIONDIGITAL_BUILD_TARGET=aliyun-hk` 后构建不可变香港发布包。
...
Sites 预览是独立、非阻塞步骤，只有用户明确要求时才能发布；不得插入任何生产 DNS
操作，也不能代替香港构建、部署、DNS 与公网验收步骤。
...
节点复核；所有成功解析的节点都应到 `47.82.66.79`，不得出现 Cloudflare 403。
...
```powershell
curl.exe -fsSI --resolve "fusiondigital.club:443:47.82.66.79" https://fusiondigital.club/
curl.exe -fsSI --resolve "www.fusiondigital.club:443:47.82.66.79" https://www.fusiondigital.club/
```
...
- 只有用户明确要求预览时才能保存或发布 Sites 版本；生产发布不依赖 Sites 状态。
...
2. 将 `@`、`www` 的所有线路恢复为 `47.82.66.79`，删除冲突的 CNAME/AAAA/A；
3. 保留验证 TXT，等待旧 TTL；
